...
int main() {
	...
	std::shared_ptr<CurrentConditionsDisplay> current(new CurrentConditionsDisplay(weather_data));
	weather_data->registerObserver(current);
	weather_data->setMeasurements(80,65,0.301);
	weather_data->setMeasurements(82, 70, 0.292);
	std::shared_ptr<CurrentConditionsDisplay> new_curr(new CurrentConditionsDisplay(weather_data));
